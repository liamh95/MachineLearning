...
X = [ones(m,1) X];
% ----- One-Hot ----- %
% each COL is a training example
examples = size(y,1);
Y = zeros(num_labels, examples);
for i = 1:examples
	Y(y(i,1),i)=1;
endfor

%%%%%%%%%% Forward Prop(unregularized) %%%%%%%%%%
a2 = sigmoid(X*Theta1');
ext = size(a2, 1);
a3 = sigmoid([ones(ext,1) a2]*Theta2');
mat = -Y'.*log(a3) - (ones(examples, num_labels)-Y').*log(ones(examples,num_labels)-a3 );
J = (1/m)*sum(mat(:));
